...
errType udp_port::open_port(bool listen_mode)
{
    ...
    if (listen_mode) {
        ...
        if (!RAW) {
    	    ...
    	    int rc=1; 
    	    setsockopt(sock,IPPROTO_IP,IP_HDRINCL,&rc,sizeof(rc)); 
    	    //rc=0;
    	    //setsockopt(sock,SOL_SOCKET,SO_BROADCAST,&rc,sizeof(rc));
    	    //rc=1;
    	    //setsockopt(sock, IPPROTO_IP, IP_DONTFRAG, &rc, sizeof(rc));
    	    if (verbose_level) printf("Create raw socket.Socket #%d\n",sock);
        }
