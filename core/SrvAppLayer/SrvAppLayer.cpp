...
errType SrvAppLayer::StartListening()
{
    ...
    if (!AppTerminated)
        ret = pthread_create(&listenerThread, NULL, _ss_udpListenerThread, (void*) this);
    ...
    if ((!AppTerminated) && (ret == 0))
        ret = pthread_create(&senderThread, NULL, udpSenderThread, (void*) this);

    /// @brief 6. Start \ref equipListenPolling
    /// @details Thread listen for udp messages from equipment

    if ((!AppTerminated) && (ret == 0))
        ret = pthread_create(&equipListenThread, NULL, equipListenPolling, (void*) this);
...
errType SrvAppLayer::equip_reading_event(DWORD timeOut_sec, DWORD timeOut_ms){
    ...
    if ((result == err_result_ok) && ((event&0x1) == 0x1))
