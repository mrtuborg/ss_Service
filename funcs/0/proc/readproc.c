...
static proc_t* simple_readproc(PROCTAB *restrict const PT, proc_t *restrict const p) {
	...
	memcpy(p->euser,   _procps_user_from_uid(p->euid), sizeof p->euser);
        if(flags & PROC_FILLSTATUS) {
            memcpy(p->ruser,  _procps_user_from_uid(p->ruid), sizeof p->ruser);
            memcpy(p->suser,  _procps_user_from_uid(p->suid), sizeof p->suser);
            memcpy(p->fuser,  _procps_user_from_uid(p->fuid), sizeof p->fuser);
        }
    }

    /* some number->text resolving which is time consuming and kind of insane */
    if (flags & PROC_FILLGRP){
        memcpy(p->egroup, _procps_group_from_gid(p->egid), sizeof p->egroup);
        if(flags & PROC_FILLSTATUS) {
            memcpy(p->rgroup, _procps_group_from_gid(p->rgid), sizeof p->rgroup);
            memcpy(p->sgroup, _procps_group_from_gid(p->sgid), sizeof p->sgroup);
            memcpy(p->fgroup, _procps_group_from_gid(p->fgid), sizeof p->fgroup);
        }
    }
...
//printf("iii\n");
    ...
    if (flags & PROC_FILLUSR){
	memcpy(t->euser,   _procps_user_from_uid(t->euid), sizeof t->euser);
        if(flags & PROC_FILLSTATUS) {
            memcpy(t->ruser,   _procps_user_from_uid(t->ruid), sizeof t->ruser);
            memcpy(t->suser,   _procps_user_from_uid(t->suid), sizeof t->suser);
            memcpy(t->fuser,   _procps_user_from_uid(t->fuid), sizeof t->fuser);
        }
    }

    /* some number->text resolving which is time consuming */
    if (flags & PROC_FILLGRP){
        memcpy(t->egroup, _procps_group_from_gid(t->egid), sizeof t->egroup);
        if(flags & PROC_FILLSTATUS) {
            memcpy(t->rgroup, _procps_group_from_gid(t->rgid), sizeof t->rgroup);
            memcpy(t->sgroup, _procps_group_from_gid(t->sgid), sizeof t->sgroup);
            memcpy(t->fgroup, _procps_group_from_gid(t->fgid), sizeof t->fgroup);
        }
    }
...
proc_t** readproctab(int flags, ...) {
	...
	//mrt: tab[n] = readproc_direct(PT, NULL);     /* final null to terminate */
...
proc_data_t *readproctab2(int(*want_proc)(proc_t *buf), int(*want_task)(proc_t *buf), PROCTAB *restrict const PT) {
    ...
    for(;;){
        ...
        //mrt: tmp = readproc_direct(PT, data+n_used);
	    ...
	    tmp = data+(tmp-old);
          ...
          //mrt: t = readtask_direct(PT, tmp, data+n_used);
